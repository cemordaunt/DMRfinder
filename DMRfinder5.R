...
cat("\n[DMRfinder] A pipeline to identify differentially-methylated regions from whole-genome bisulfite sequencing data\n")
...
permuteAll <- function(nperm, design) {
        ...
        message(sprintf("[DMRfinder] performing %d unrestricted permutations of the design matrix\n", nperm))
...
getNullDistribution_BSmooth.tstat <- function(BSseq, idxMatrix1, idxMatrix2, estimate.var, local.correct = TRUE, cutoff, stat = "tstat.corrected", maxGap, mc.cores) {
        ...
        message(sprintf("[DMRfinder] Performing %d permutations", nrow(idxMatrix1)-1)) 
        nullDist <- lapply(1:nrow(idxMatrix1), function(ii) {
                ...
                message(sprintf("[DMRfinder] Completing permutation %d in %.1f sec", ii-1, stime))
                dmrs0
        })
        nullDist
}
...
plotManyRegions2 <- function (BSseq, regions = NULL, extend = 0, main = "", addRegions = NULL, annoTrack = NULL, cex.anno = 1, geneTrack = NULL, cex.gene = 1.5, 
        ...
        for (ii in seq(along = gr)) {
                if (verbose) cat(sprintf("[DMRfinder] plotting region %d (out of %d)\n", ii, nrow(regions)))
...
packageLoad <- function(packages, lib = "/share/lasallelab/programs/DMRfinder/R_3.5"){
        # Originally written by Ben Laufer as part of the DMRichR package
        # Modified by Charles Mordaunt
        # Installs and loads desired packages
        CRAN <- c("BiocManager", "remotes", "magrittr")
        new.CRAN.packages <- CRAN[!(CRAN %in% installed.packages(lib = lib)[,"Package"])]
        if(length(new.CRAN.packages) > 0){install.packages(new.CRAN.packages, repos ="https://cloud.r-project.org", quiet = TRUE, lib = lib)}
        stopifnot(suppressMessages(sapply(CRAN, require, character.only = TRUE)))
        new.packages <- packages[!(packages %in% installed.packages()[,"Package"])]
        if(length(new.packages)>0){BiocManager::install(new.packages, ask = FALSE, quiet = TRUE, lib = lib)}
...
# Make sure optparse is installed
packageLib <- "/share/lasallelab/programs/DMRfinder/R_3.5"
.libPaths(packageLib)
if(!"optparse" %in% installed.packages(lib = packageLib)){install.packages("optparse", lib = packageLib)}

# Get arguments from bash script
cat("[DMRfinder] Getting arguments from bash script\n\n")
option_list <- list(
        # Required arguments
        optparse::make_option(opt_str = c("-n", "--chrNum"), type = "integer", default = NULL, help = "chromosome number"),
        optparse::make_option(opt_str = c("-d", "--setwd"), type = "character", default = NULL, help = "working directory"),
        optparse::make_option(opt_str = c("-c", "--numCtrl"), type = "integer", default = NULL, help = "number of control samples"),
        optparse::make_option(opt_str = c("-e", "--numExp"), type = "integer", default = NULL, help = "number of experimental samples"),
        optparse::make_option(opt_str = c("-a", "--genome"), type = "character", default = NULL, help = "genome assembly (hg38, hg19, mm10, rn6, rheMac8)"),
        optparse::make_option(opt_str = c("-o", "--outprefix"), type = "character", default = NULL, help = "title used in output files"),
        
        # Optional arguments
        optparse::make_option(opt_str = c("--pctMinCtrl"), type = "double", default = 0.9, help = "minimum percent of control samples with 1 read at CpG [default = 0.9]"),
        optparse::make_option(opt_str = c("--pctMinExp"), type = "double", default = 0.9, help = "minimum percent of experimental samples with 1 read at CpG [default = 0.9]"),
        optparse::make_option(opt_str = c("--mc.cores"), type = "integer", default = 1, help = "cores to use, same as SBATCH -n [default = 1]"),
        optparse::make_option(opt_str = c("--estimate.var"), type = "character", default = "same", help = "method to estimate variance for t-test (same, paired, group2) [default = same]"),
        optparse::make_option(opt_str = c("--meanDiff_cutoff"), type = "double", default = 0.05, help = "minimum difference between group means for DMRs [default = 0.05]"),
        optparse::make_option(opt_str = c("--maxGap"), type = "integer", default = 300, help = "maximum distance between all consecutive CpGs in a DMR [default = 300]"),
        optparse::make_option(opt_str = c("--invdensity_cutoff"), type = "integer", default = 300, help = "maximum average distance between consecutive CpGs in a DMR [default = 300]"),
        optparse::make_option(opt_str = c("--nperm"), type = "integer", default = 1000, help = "number of permutations to perform for FWER estimation [default = 1000]"),
        
        # Output arguments
        optparse::make_option(opt_str = c("--silver_bed"), type = "logical", default = TRUE, help = "Output bed file of locations for silver DMRs [default = TRUE]"),
        optparse::make_option(opt_str = c("--silver_info"), type = "logical", default = TRUE, help = "Output txt file of info for silver DMRs [default = TRUE]"),
        optparse::make_option(opt_str = c("--gold_plots"), type = "logical", default = TRUE, help = "Output pdf file of plots for gold DMRs [default = TRUE]"),
        optparse::make_option(opt_str = c("--background"), type = "logical", default = TRUE, help = "Output bed file of background DMRs [default = TRUE]"),
        optparse::make_option(opt_str = c("--gold_bed"), type = "logical", default = FALSE, help = "Output bed file of locations for gold DMRs [default = FALSE]"),
        optparse::make_option(opt_str = c("--silver_plots"), type = "logical", default = FALSE, help = "Output pdf file of plots for silver DMRs [default = FALSE]"),
        optparse::make_option(opt_str = c("--meth"), type = "logical", default = FALSE, help = "Output table of smoothed methylation for each sample at each silver DMR [default = FALSE]"),
        optparse::make_option(opt_str = c("--cov"), type = "logical", default = FALSE, help = "Output table of total coverage for each sample at each DMR [default = FALSE]"),
        optparse::make_option(opt_str = c("--background_cov"), type = "logical", default = FALSE, help = "Output table of total coverage for each sample at each background DMR [default = FALSE]"),
        optparse::make_option(opt_str = c("--CpGs"), type = "logical", default = FALSE, help = "Output bed file of tested CpGs [default = FALSE]")
)
opt_obj <- optparse::OptionParser(option_list = option_list, epilogue = "\nAdd DSS_file prefixes after all other arguments, with all control samples first (DSS_files/CTRL01_)\n", 
                        add_help_option = TRUE, usage = "usage: %prog [arguments]")
opt <- optparse::parse_args(object = opt_obj, positional_arguments = c(0, Inf), print_help_and_exit = FALSE)

# Test for required arguments
if(opt$options$help){print_help_compact(opt_obj); stop("", call.=FALSE)}
if(is.null(opt$options$chrNum)){print_help_compact(opt_obj); stop("chrNum must be supplied\n\n", call.=FALSE)}
if(is.null(opt$options$setwd)){print_help_compact(opt_obj); stop("setwd must be supplied\n\n", call.=FALSE)}
if(is.null(opt$options$numCtrl)){print_help_compact(opt_obj); stop("numCtrl must be supplied\n\n", call.=FALSE)}
if(is.null(opt$options$numExp)){print_help_compact(opt_obj); stop("numExp must be supplied\n\n", call.=FALSE)}
if(is.null(opt$options$genome)){print_help_compact(opt_obj); stop("genome must be supplied\n\n", call.=FALSE)}
if(is.null(opt$options$outprefix)){print_help_compact(opt_obj); stop("outprefix must be supplied\n\n", call.=FALSE)}
if(length(opt$args) < 4){print_help_compact(opt_obj); stop("At least 4 DSS files must be supplied\n\n", call.=FALSE)}
cat("\n", str(opt))   
...
DSSprefix <- opt$args   
colorCtrl <- as.character("#3366CC")             
colorExp <- as.character("#FF3366") 
...
packageLoad(packages = c("BiocGenerics", "Biobase", "S4Vectors", "matrixStats", "DelayedArray", "bsseq", "DSS", "permute",
            "GenomicRanges", "scales"), lib = packageLib)
...
rm(DSSlist)
BSobj_smoothed <- BSmooth(BSseq = BSobj, ns = 70, h = 1000, maxGap = 10^8, keep.se = FALSE,
                          BPPARAM = MulticoreParam(workers = mc.cores, progressbar = TRUE), verbose = TRUE)    
cat("[DMRfinder] Completed smoothing\n")
...
BSobj_tstat <- BSmooth.tstat(BSobj_keep, group1 = EXPgroup, group2 = CTRLgroup, estimate.var = estimate.var, local.correct = TRUE, qSd = 0.75, k = 101, 
                             maxGap = 10^8, mc.cores = mc.cores, verbose = FALSE)    
