...
option_list <- list(
        ...
        make_option(opt_str = c("-r", "--regions"), type = "character", default = NULL, help = "bed file of predefined regions with header, (chr, start, end) [required]"),
        ...
        make_option(opt_str = c("-g", "--genome"), type = "character", default = NULL, help = "genome assembly (hg38, hg19, mm10, rn6, rheMac8)"),
        ...
        # Output arguments
        make_option(opt_str = c("--meth"), type = "logical", default = TRUE, help = "Output table of smoothed methylation for each sample at each silver DMR [default = TRUE]"),
...
if(is.null(opt$options$regions)){
        print_help_compact(opt_obj)
        stop("Regions must be supplied\n\n", call.=FALSE)
}
...
# Load DMR file
chrom <- chroms[chrNum]
cat("\n[DMRmethyl] Loading", chrom, "Regions\n")
DMRs <- read.delim(file = as.character(opt$options$regions), header = FALSE, sep = "\t")
DMRs <- DMRs[2:nrow(DMRs),]
...
if(dim(DMRs)[1] > 0){
        ...
        if(meth){
                cat("\n[DMRmethyl] Printing region methylation\n")
        ...
        if(cov){
                cat("\n[DMRmethyl] Printing region coverage\n")
